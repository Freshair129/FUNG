...
- Browser click-through reached Home → Live → Review, rail expansion, appearance controls and the account sheet at the local desktop surface; source/build/regression checks pass.
...
Approval was recorded in the task conversation on 2026-09-19 before implementation. The remaining native click-through boundary requires an interactive non-elevated desktop session and is not silently treated as passed.
